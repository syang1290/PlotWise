...
function App() {
 ...
 const downloadPDF = async () => {
  ...
  try {
    // Force the element to be "visible" but hidden far off-screen
    ...
    const dataUrl = await toPng(element, { 
      ...
      width: 800,
      pixelRatio: 2 // Increases clarity for text
    });
    ...
    element.style.display = 'none'; // Re-hide it
    ...
    <div className="flex h-screen w-full bg-[#F8FAFC] overflow-hidden antialiased text-slate-900" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <div className="relative flex-1 bg-slate-200">
        <div className="absolute top-6 left-6 z-10 w-[400px]">
          <div className="bg-white rounded-lg shadow-sm border border-slate-300 flex items-center p-1">
            ...
            />
            <button onClick={handleSearch} disabled={isLoading} className="px-5 py-2 rounded-md bg-slate-900 text-white text-xs font-extrabold hover:bg-black transition-all uppercase tracking-tighter">
              {isLoading ? '...' : 'Search'}
            </button>
          </div>
        </div>
        ...
        {propertyData && (
          <div className="absolute bottom-6 left-6 right-6 z-10 flex flex-col items-center">
            {showChat && chatAnswer && (
              <div className="mb-4 w-full max-w-2xl bg-white p-6 rounded-xl border border-slate-200 shadow-2xl animate-in slide-in-from-bottom-4 duration-300">
            ...
            <div className="w-full max-w-xl relative">
              <input 
                ...
                className="w-full py-5 pl-6 pr-14 rounded-xl bg-white shadow-2xl border border-slate-200 outline-none text-sm font-semibold tracking-tight focus:ring-2 focus:ring-slate-900/5 transition-all"
              />
      ...
      <div className="w-[480px] bg-white flex flex-col border-l border-slate-200 overflow-hidden shadow-[-10px_0_15px_-3px_rgba(0,0,0,0.03)]">
        <header className="p-10 border-b border-slate-100 flex justify-between items-center bg-white">
          ...
          {propertyData && (
            <button onClick={downloadPDF} disabled={isDownloading} className="flex items-center gap-2 px-5 py-2.5 bg-slate-900 rounded-md text-[10px] font-extrabold text-white hover:bg-black transition-all tracking-widest">
              {isDownloading ? <Loader2 className="h-3 w-3 animate-spin" /> : <Download className="h-3 w-3" />} EXPORT
            </button>
          )}
        </header>
        ...
        <nav className="flex px-10 bg-white border-b border-slate-100">
          {['Overview', 'Zoning (ADU)', 'Inclusivity'].map(tab => (
            <button key={tab} onClick={() => setActiveAnalysis(tab)} className={`py-5 px-4 text-[11px] font-extrabold uppercase tracking-widest transition-all border-b-[3px] ${activeAnalysis === tab ? 'border-slate-900 text-slate-900' : 'border-transparent text-slate-300 hover:text-slate-500'}`}>
              {tab}
            </button>
          ))}
        </nav>
                ...
                <div className="space-y-8">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-5 border border-slate-100 rounded-xl bg-white shadow-sm">
                      <p className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest mb-1">Est. Market Value</p>
                      <p className="text-lg font-bold text-slate-900 tracking-tight">{propertyData.estimated_value}</p>
                    </div>
                    <div className="p-5 border border-slate-100 rounded-xl bg-white shadow-sm">
                      <p className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest mb-1">Lot Square Footage</p>
                      <p className="text-lg font-bold text-slate-900 tracking-tight">{propertyData.lot_size}</p>
                    </div>
                    <div className="p-5 border border-slate-100 rounded-xl bg-white shadow-sm">
                      <p className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest mb-1">Year Built</p>
                      <p className="text-lg font-bold text-slate-900 tracking-tight">{propertyData.year_built}</p>
                    </div>
                    <div className="p-5 border border-slate-100 rounded-xl bg-white shadow-sm">
                      <p className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest mb-1">Last Transaction</p>
                      <p className="text-lg font-bold text-slate-900 tracking-tight">{propertyData.last_sale}</p>
                    </div>
                  </div>
                  ...
                  {/* RESTORED: Strategic Feasibility Tasks */}
              ...
              {activeAnalysis === 'Zoning (ADU)' && (
                <div className="space-y-6">
                  <div className="p-8 border-2 border-slate-900 rounded-xl bg-white shadow-sm">
                    <h3 className="text-[11px] font-extrabold text-slate-900 mb-8 uppercase tracking-widest">Structural Guidelines</h3>
                    <div className="grid grid-cols-2 gap-x-10 gap-y-8">
                      <div>
                        <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-2">Max Elevation</p>
                        <p className="text-lg font-extrabold text-slate-900 tracking-tight">{propertyData.zoning_details?.max_height || 'N/A'}</p>
                      </div>
                      <div>
                        <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-2">Lot Coverage</p>
                        <p className="text-lg font-extrabold text-slate-900 tracking-tight">{propertyData.zoning_details?.lot_coverage || 'N/A'}</p>
                      </div>
                      <div>
                        <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-2">Setbacks</p>
                        <p className="text-lg font-extrabold text-slate-900 tracking-tight">{propertyData.zoning_details?.setbacks || 'N/A'}</p>
                      </div>
                      <div>
                        <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-2">Parking Req.</p>
                        <p className="text-lg font-extrabold text-slate-900 tracking-tight">{propertyData.zoning_details?.parking_req || 'N/A'}</p>
                      </div>
                    </div>
                  </div>
                  <div className="p-8 border border-slate-100 bg-slate-50 rounded-xl">
                    <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-4">ADU Regulatory Notes</p>
                ...
                <div className="space-y-8">
                  <div className="p-8 border border-slate-200 rounded-xl bg-white shadow-sm">
                    <div className="flex justify-between items-center mb-8">
                      <h3 className="text-[11px] font-extrabold text-slate-900 uppercase tracking-widest">Mobility Index</h3>
                      <span className="text-3xl font-extrabold text-slate-900 tracking-tighter">{propertyData.inclusivity_score}%</span>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-5 border border-slate-100 rounded-xl bg-slate-50/50">
                      <p className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest mb-1">Transit Score</p>
                      <p className="text-lg font-bold text-slate-900">{propertyData.inclusivity_details?.transit_score || 'N/A'}/100</p>
                    </div>
                    <div className="p-5 border border-slate-100 rounded-xl bg-slate-50/50">
                      <p className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest mb-1">Walk Score</p>
                      <p className="text-lg font-bold text-slate-900">{propertyData.inclusivity_details?.walk_score || 'N/A'}/100</p>
                    </div>
                    <div className="p-5 border border-slate-100 rounded-xl bg-slate-50/50">
                      <p className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest mb-1">Pedestrian Safety</p>
                      <p className="text-lg font-bold text-slate-900">{propertyData.inclusivity_details?.pedestrian_safety || 'N/A'}</p>
                    </div>
                    <div className="p-5 border border-slate-100 rounded-xl bg-slate-50/50">
                      <p className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest mb-1">Terrain Profile</p>
                      <p className="text-lg font-bold text-slate-900">{propertyData.inclusivity_details?.topography || 'Standard'}</p>
                    </div>
                  </div>

                  <div className="p-8 border border-slate-200 rounded-xl bg-white shadow-sm">
                    <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-3">Affordable Housing Eligibility</p>
                    <p className="text-lg font-extrabold text-slate-900 tracking-tight mb-2">{propertyData.inclusivity_details?.affordable_housing_bonus ? 'Bonus Active' : 'No Bonus Found'}</p>
                    <p className="text-sm text-slate-500 font-medium leading-relaxed">{propertyData.inclusivity_details?.affordable_housing_bonus}</p>
                  </div>
                </div>
              )}
            </div>
        ...
        {/* RESTORED: Action Protocol Modal Overlay */}
        {selectedAction && (
...
{propertyData && (
  <div 
    ...
    style={{ 
      ...
      backgroundColor: '#ffffff', 
      color: '#0f172a',
      minHeight: '1100px' // Ensures it fills an A4 page
    }}
  >
    {/* Header */}
    <div style={{ borderBottom: '4px solid #0f172a', paddingBottom: '40px', marginBottom: '40px' }}>
      <h1 style={{ fontSize: '32px', fontWeight: '800', margin: '0 0 10px 0' }}>PlotWise Technical Registry</h1>
      <p style={{ color: '#64748b', fontSize: '12px', fontWeight: '800', textTransform: 'uppercase' }}>
        Verified Parcel Data • {propertyData.last_updated}
      </p>
    </div>

    {/* Summary Box */}
    <div style={{ marginBottom: '40px' }}>
      <h2 style={{ fontSize: '12px', fontWeight: '800', color: '#94a3b8', textTransform: 'uppercase', marginBottom: '15px' }}>Property Address</h2>
      <p style={{ fontSize: '24px', fontWeight: '800', margin: '0 0 20px 0' }}>{propertyData.address}</p>
      <div style={{ padding: '30px', backgroundColor: '#f8fafc', borderRadius: '12px', border: '1px solid #f1f5f9' }}>
        <p style={{ fontSize: '16px', lineHeight: '1.6', fontWeight: '600' }}>{propertyData.summary}</p>
      </div>
    </div>
    ...
    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
      <tbody>
        <tr style={{ borderBottom: '1px solid #f1f5f9' }}>
          <td style={{ padding: '20px 0', fontWeight: '700', color: '#64748b' }}>Market Valuation</td>
          <td style={{ padding: '20px 0', fontWeight: '800', textAlign: 'right' }}>{propertyData.estimated_value}</td>
        </tr>
        <tr style={{ borderBottom: '1px solid #f1f5f9' }}>
          <td style={{ padding: '20px 0', fontWeight: '700', color: '#64748b' }}>Lot Area</td>
          <td style={{ padding: '20px 0', fontWeight: '800', textAlign: 'right' }}>{propertyData.lot_size}</td>
        </tr>
        <tr style={{ borderBottom: '1px solid #f1f5f9' }}>
          <td style={{ padding: '20px 0', fontWeight: '700', color: '#64748b' }}>Zoning Code</td>
          <td style={{ padding: '20px 0', fontWeight: '800', textAlign: 'right' }}>{propertyData.zoning_code}</td>
        </tr>
        <tr>
          <td style={{ padding: '20px 0', fontWeight: '700', color: '#64748b' }}>Inclusivity Score</td>
          <td style={{ padding: '20px 0', fontWeight: '800', textAlign: 'right' }}>{propertyData.inclusivity_score}%</td>
        </tr>
      </tbody>
    </table>
  </div>
